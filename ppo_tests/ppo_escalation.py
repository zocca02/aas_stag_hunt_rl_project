...
arg_parser.add_argument("-n", "--name", type=str, default="ppo_escalation", help="Generic name")
...
looper = EscalationLooper(args.n_envs, env_params, parallelization="async" if args.async_envs else "sync", print_rewards=PRINT_REWARDS, test=args.test,
                        evaluate_during_training=False, evaluate_every_n_steps=args.time_steps, n_evaluations=4, add_streak_len=args.streak_len, max_streak_len=args._get_args.time_steps)
...
critic_net1 = CriticNet(state_shape[0])
policy_net1 = PolicyNet(state_shape[0], n_actions)
...
agent1 = PPOAgent(model_wrapper1, args.n_envs, args.rollout_steps, state_shape, args.batch_size, 
                  ...
                  device=device, test=args.test, greedy_on_test=True)

########################
# AGENT2
########################

critic_net2 = CriticNet(state_shape[0])
policy_net2 = PolicyNet(state_shape[0], n_actions)
